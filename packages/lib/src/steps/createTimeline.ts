...
const getWives = (tree: KnowledgeTree, patriarch: string): Timeline[] => {
    ...
    for (const wife in tree[patriarch].marriages) {
        ...
        timeline.otherMarriages = getOtherMarriages(tree, wife, patriarch)
            .sort((marriageA, marriageB) => marriageA.start.getTime() - marriageB.start.getTime())
        timelines.push(timeline)
    }
    return timelines
}

export const createTimeline = (tree: KnowledgeTree, patriarch): { rootTimeline: PatriarchTimeline, wives: Timeline[] } => {
    ...
    const wives = getWives(tree, patriarch)
        .sort((wifeA,wifeB) => {
