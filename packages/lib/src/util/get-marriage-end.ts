...
export const getMarriageEnd = (
    ...
    if (tree[wife].marriages) {
        ...
        const sortedMarriages = Object.values(tree[wife].marriages).sort(
            (marriageA, marriageB) => {
        ...
        for (let i = 0; i < sortedMarriages.length; i++) {
            ...
            if (!subsequentMarriage) {
                ...
                return isBefore(wifesDeath!, husbandDeath!)
                    ? wifesDeath
                    : husbandDeath
            }
            ...
            if (
                husbandDeath &&
                subsequentMarriage.date &&
                isBefore(husbandDeath, subsequentMarriage.date)
            ) {
                return husbandDeath
            }
